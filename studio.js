...
function viewHash(){const p=new URLSearchParams({v:'1',fig:state.figure,tech:[...state.tech].join(','),mode:state.mode,node:state.node,from:state.from,to:state.to,labels:state.labels?'1':'0',scale:state.scale,energy:state.energy,x:state.x,y:state.y,xscale:state.xscale,pins:state.pins.join(',')});if(state.selected!==null)p.set('selected',state.selected);return '#'+p.toString();}
...
function syncStudio(){
 $('explore-controls').hidden=state.figure!=='custom';$('energy-controls').hidden=state.figure!=='7b';
...
function renderStudio(){syncStudio();renderComparison();document.querySelectorAll('[data-pin]').forEach(b=>b.onclick=e=>{e.stopPropagation();togglePin(Number(b.dataset.pin));});history.replaceState(null,'',location.pathname+location.search+viewHash());}
function wireStudio(){
